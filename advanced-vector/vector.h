...
public:
    ...
    iterator Emplace(const_iterator pos, Args&&... args)
    {
        ...
        {
            AllocateAndMoveData(position, std::forward<Args>(args)...);
        }
        else 
        {
            try {
                InsertIntoExistingBuffer(pos, position, std::forward<Args>(args)...);
            }
            catch (...) {
                HandleExceptionDuringInsert();
            }
        }
        size_++;
...
private:
    ...
    void MoveOrCopyElements(T* data, T* new_data, int position)
    {
        if constexpr (std::is_nothrow_move_constructible_v<T> || !std::is_copy_constructible_v<T>){
            std::uninitialized_move_n(data, position, new_data);
        }
        else {
            std::uninitialized_copy_n(data, position, new_data);
        }
    }

    template <typename... Args>
    void AllocateAndMoveData(int position, Args&&... args)
    {
        RawMemory<T> new_data(size_ == 0 ? 1 : size_ * 2);
        new (new_data.GetAddress() + position) T(std::forward<Args>(args)...);
        MoveOrCopyElements(data_.GetAddress(), new_data.GetAddress(), position);
        MoveOrCopyElements(data_.GetAddress() + position, new_data.GetAddress() + position + 1, size_ - position);
        DestroyN(data_.GetAddress(), size_);
        data_.Swap(new_data);
    }

    template <typename... Args>
    void InsertIntoExistingBuffer(const_iterator pos,int position, Args&&... args)
    {
        if (pos != end()) 
        {
    ...
    void HandleExceptionDuringInsert() 
    {
        operator delete (end());
        throw;
    }
